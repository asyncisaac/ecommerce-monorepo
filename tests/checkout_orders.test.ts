...
describe('Checkout & Orders', () => {
  it('adiciona itens ao carrinho, faz checkout, lista e detalha ordens, e valida estoque/carrinho', async () => {
    ...
    // Summary do carrinho
    const summary = await request(base).get('/api/cart/summary').set(headers);
    expect(summary.statusCode).toBe(200);
    expect(summary.body?.itemsCount).toBeGreaterThan(0);
    expect(summary.body?.total).toBeGreaterThan(0);

    // Checkout
    const checkout = await request(base).post('/api/checkout').set(headers);
    ...
    // Listar ordens
    const orders = await request(base).get('/api/orders').set(headers);
    ...
    // Detalhar ordem
    const byId = await request(base).get(`/api/orders/${order.id}`).set(headers);
    expect(byId.statusCode).toBe(200);
    expect(byId.body?.id).toBe(order.id);

    // Carrinho deve estar vazio
    const cart = await request(base).get('/api/cart').set(headers);
    expect(cart.statusCode).toBe(200);
    expect((cart.body?.items || []).length).toBe(0);

    // Estoque do produto deve ter diminuído (2 unidades)
    const prodAfter = await request(base).get(`/api/products/${prod.id}`).set(headers);
